...
for L = 0:Lmax
   ...
   luxchain, ps, st = equivariant_model(totdeg, ν, radial, L;islong = false, isState = true)
   F(X) = luxchain(X, ps, st)[1]

   luxchain2, ps2, st2 = equivariant_model(EquivariantModels.degord2spec(radial;totaldegree=totdeg,order=ν,Lmax=L,islong = true)[2][1:end-1],radial,L;islong = false, isState = true)  
...
basis = legendre_basis(totdeg)
radial = EquivariantModels.simple_radial_basis(basis)
luxchain, ps, st = equivariant_model(totdeg,ν,radial,L;islong = true, isState = true)
F(X) = luxchain(X, ps, st)[1]
luxchain2, ps2, st2 = equivariant_model(EquivariantModels.degord2spec(radial;totaldegree=totdeg,order=ν,Lmax=L,islong = true)[2][1:end-1],radial,L;islong = true, isState = true)
F2(X) = luxchain(X, ps2, st2)[1]
...
for L = 0:Lmax
   ...
   luxchain, ps, st = equivariant_model(nn,ll,radial,L;islong = false, isState = true) 
   F(X) = luxchain(X, ps, st)[1]
